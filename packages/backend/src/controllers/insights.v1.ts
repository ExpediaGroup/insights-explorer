...
/**
 * GET /insights/:namespace/:name
 */
export const getInsight = async (req: Request, res: Response): Promise<void> => {
  logger.debug(`[INSIGHTS.V1] Attempting to load insight: ${req.params.namespace}/${req.params.name}`);

  const result = await client
    .query(
      `
        query InsightByName($fullName: String!) {
      ...
      `,
      { fullName: `${req.params.namespace}/${req.params.name}` }
    )
    .toPromise();

  if (result.error) {
    logger.error(`[INSIGHTS.V1] GraphQL error: ${result.error}`);
    res.status(500).send();
  } else if (result.data.insight == null) {
    res.status(404).send();
  } else {
    res.send(result.data.insight);
  }
};
...
export const search = async (req: Request, res: Response): Promise<void> => {
  logger.debug(`[SEARCH.V1] Attempting to search: ${req.query.q}`);

  if (req.query.q === undefined) {
    res.status(500).send("Query parameter 'q' is required ");
    return;
  }
  ...
  if (result.error) {
    logger.error(`[INSIGHTS.V1] GraphQL error: ${result.error}`);
    res.status(500).send();
  } else if (result.data.insights == null) {
    res.status(404).send();
  } else {
    res.send(result.data.insights);
  }
};
